...
import sword.okhttp.library.SwordAddress
import sword.okhttp.library.SwordConnectionPool
import sword.okhttp.library.SwordRealCall
...
class SwordExchangeFinder(
    ...
    private fun findHealthConnection(): SwordConnection {
        ...
        while (true) {
            ...
            //todo：创建一个新的连接, 然后请求 TCP ，然后基于 TCP 再建立 HTTP、HTTP2/3 或 HTTPS 的连接
            //todo：再次尝试从连接池中获取且仅获取一个可用的 HTTP2/3 连接，如果拿到了，就将新创建的丢弃掉
