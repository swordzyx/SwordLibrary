...
class TraceTransform : Transform() {
  ...
  override fun transform(transformInvocation: TransformInvocation) {
    Logger.debug("TraceTransform transform")
  ...
  private fun traceJarFiles(jarInput: JarInput, outputProvider: TransformOutputProvider) {
    ...
    JarFile(jarInput.file).use { jarFile ->
      ...
      //拷贝修改后的 jar 文件到目标目录
      val targetJarPath = outputProvider.getContentLocation(
        jarInput.name,
        jarInput.contentTypes,
        jarInput.scopes,
        Format.JAR
      )
      Logger.debug("拷贝 ${tempFile.absolutePath} 到 $targetJarPath")
      FileUtilsKt.copyFile(
        tempFile,
        targetJarPath
      )
    }
