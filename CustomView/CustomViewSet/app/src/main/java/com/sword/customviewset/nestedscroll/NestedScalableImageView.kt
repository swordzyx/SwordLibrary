package com.sword.customviewset.nestedscroll
...
class NestedScalableImageView(context: Context?, attrs: AttributeSet?) : View(context, attrs),
    ...
    //本次滑动中，子 View 已经滑动一定的距离了，再次询问父 View 是否需要优先滑动。这是第二次询问
    override fun dispatchNestedScroll(
            dxConsumed: Int,
            dyConsumed: Int,
            dxUnconsumed: Int,
    ...
    //滑动开始时，询问父 View 本次滑动是否需要优先滑动，这是第一次询问
    ...
    inner class HenGestureListener : GestureDetector.SimpleOnGestureListener() {
        ...
        override fun onScroll(
            ...
            if (big) {
                ...
                if (unconsumed != 0) {
                    //询问父 View 是否需要滑动。滑动开始时则不需要去询问父 View ，因为此时滑动的控制权已经由父 View 转交给子 View 了，因此在滑动开始时的第一次询问就不需要了，直接开始滑动即可。
